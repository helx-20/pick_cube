...
stage1_collect.py saves each rollout as a dict with per-step lists:
    {
      "obs":     [obs_0, obs_1, ...],          # each obs is shape (42,)
...
def flatten_episodes(
    ...
    Returns (X, y) with X shape (N, 45), y shape (N,).
    """
    ...
    if not X_chunks:
        return np.zeros((0, 45), dtype=np.float32), np.zeros((0,), dtype=np.int64)
