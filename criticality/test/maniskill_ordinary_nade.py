...
@dataclass
class NADEConfig:
    # PickCube: state obs is 42-dim
    base_feature_dim: int = 42
    env_param_dim: int = 3
...
class ManiSkillOrdinaryNADE(gym.Wrapper):
    def __init__(self, env, cfg, args):
        ...
        # PickCube: input_dim = 42 (obs) + 3 (force fx,fy,fz) = 45
        input_dim = 45
        ...
        # PickCube: 11^3 = 1331 discrete (fx, fy, fz) candidates,
    ...
    def calcu_q(self, obs=None):
        ...
        # over the same current observation -> batch shape (1331, 45).
        total_points = self.total_actions  # 1331

        cur_obs = torch.tensor(obs, dtype=torch.float32, device=self.device)\
            .unsqueeze(0).repeat(total_points, 1)  # (1331, 42)
        cur_input = torch.cat([cur_obs, self.force_grid], dim=1)  # (1331, 45)
    ...
    def idx_to_action(self, action_idx):
        # PickCube: action_idx ∈ [0, 1331)，直接索引 11^3 离散 (fx, fy, fz) 网格
    ...
    def step(self, action):
        if (self.step_count % self.cfg.update_every) == 0:
            ...
            if self.total_weight * criticality_info['weight'] < threshold:
                ...
                if np.random.rand() > self.cfg.force_prob:
                    ...
                    # PickCube: 从 1331 离散 (fx, fy, fz) 网格中取出该方向
        ...
        # PickCube: apply force to cube (the one being picked)
        if self.args.sim_backend == "physx_cuda":
            self.env.unwrapped.cube.apply_force(ap_force_tensor.reshape(1, 3))
        else:
            # PhysX CPU expects numpy float32 shape (3,)
            self.env.unwrapped.cube.apply_force(ap_force.astype(np.float32))
